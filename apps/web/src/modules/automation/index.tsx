...
export function AutomationModule({ initialView = 'dashboard', initialLetterFilter = 'all' }: AutomationModuleProps) {
  ...
  const navItems = [
    { icon:<Mail className="h-6 w-6"/>, label_fa:'مکاتبات', label_en:'Correspondence', view:'letters' as AutoView, color:'#3b82f6', count:2 },
    { icon:<CheckSquare className="h-6 w-6"/>, label_fa:'درخواست‌های اداری', label_en:'Admin Requests', view:'requests' as AutoView, color:'#8b5cf6', count:2 },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">⚙️ {fa ? 'اتوماسیون اداری' : 'Office Automation'}</h1>
        <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">{fa ? 'مدیریت مکاتبات، جلسات، وظایف و گردش‌کار' : 'Manage correspondence, meetings, tasks & workflows'}</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {navItems.map(item => (
          <button key={item.view} onClick={() => setView(item.view)}
            ...
            <span className="font-semibold text-sm">{fa ? item.label_fa : item.label_en}</span>
          </button>
        ))}
      </div>
