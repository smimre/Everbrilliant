...
const MODULE_ITEMS: { id: Module; label_fa: string; label_en: string; label_hi: string; icon: string; href: string }[] = [
  { id: 'trading',       label_fa: 'بازرگانی',   label_en: 'Trading',        label_hi: 'व्यापार',      icon: '🏪', href: '/trading' },
  { id: 'finance',       label_fa: 'مالی',        label_en: 'Finance',        label_hi: 'वित्त',        icon: '💰', href: '/finance' },
  { id: 'manufacturing', label_fa: 'تولید',       label_en: 'Manufacturing',  label_hi: 'विनिर्माण',   icon: '🏭', href: '/manufacturing' },
  { id: 'logistics',     label_fa: 'لجستیک',     label_en: 'Logistics',      label_hi: 'लॉजिस्टिक्स', icon: '🚛', href: '/logistics' },
  { id: 'automation',    label_fa: 'اتوماسیون',  label_en: 'Automation',     label_hi: 'स्वचालन',     icon: '⚙️', href: '/automation' },
  { id: 'crm',           label_fa: 'مشتریان',    label_en: 'CRM',            label_hi: 'सीआरएम',      icon: '🏢', href: '/crm' },
];
    ...
    <header className="h-14 shrink-0 border-b border-[hsl(var(--border))] bg-[hsl(var(--secondary))] flex items-center px-4 gap-3 z-20">
      ...
      <div className="flex items-center gap-1 flex-1 overflow-x-auto" data-tour="modules">
        {MODULE_ITEMS.map(mod => (
          <button key={mod.id}
            ...
            <span>{mod.icon}</span>
            <span className="hidden sm:inline">{lang === 'fa' ? mod.label_fa : lang === 'hi' ? mod.label_hi : mod.label_en}</span>
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        <button onClick={() => router.push('/help')}
          title={lang === 'fa' ? 'راهنما' : lang === 'hi' ? 'सहायता' : 'Help'}
